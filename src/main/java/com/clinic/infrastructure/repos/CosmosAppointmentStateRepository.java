...
public class CosmosAppointmentStateRepository implements AppointmentStateRepository {
  ...
  // Every call site in this class (save/findById/findByInsuredId/updateStatus) always returns a
  // value from Cosmos, so only the Supplier overload is ever exercised — a Runnable overload
  // existed here previously but was genuinely dead code (never invoked, not part of the
  // AppointmentStateRepository contract), so it was removed rather than tested with reflection
