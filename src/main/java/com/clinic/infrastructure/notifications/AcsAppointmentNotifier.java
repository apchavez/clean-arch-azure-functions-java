 ...
 *
 * <p>Best-effort: failures are logged but never propagated. A notification failure must not prevent
 * the appointment lifecycle from completing.
 *
 * <p>Authenticates via Managed Identity (DefaultAzureCredential). Skips silently when contactEmail
 * is absent or ACS endpoint is not configured.
 */
public class AcsAppointmentNotifier implements AppointmentNotifier {
  ...
  /** Visible for testing — lets tests inject a mocked {@link EmailClient}. */
  AcsAppointmentNotifier(EmailClient emailClient, String senderAddress) {
