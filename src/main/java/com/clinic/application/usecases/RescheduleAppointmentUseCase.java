...
public class RescheduleAppointmentUseCase {
  ...
  /**
   * Marks the existing appointment as RESCHEDULED and creates a new PENDING appointment for the new
   * schedule slot. The new appointment goes through the same event-driven flow as a fresh creation.
   */
  public Appointment execute(String appointmentId, int newScheduleId) {
    ...
    Appointment newAppointment =
        new Appointment(
            UUID.randomUUID().toString(), old.getInsuredId(), newScheduleId, old.getCountryISO());
