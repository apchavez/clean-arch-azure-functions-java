 ...
 * triggered from SQS: 1. load the appointment state (Cosmos) 2. mark it completed (domain
 * invariant) 3. persist the final record (MySQL) 4. update state to completed (Cosmos) 5. publish
 * the completed event (Event Grid)
 *
 * <p>Idempotent: markCompleted() only transitions from PENDING, so a redelivered Service Bus
 * message won't double-process (at-least-once safe).
 */
public class ProcessAppointmentUseCase {
  ...
  public ProcessAppointmentUseCase(
