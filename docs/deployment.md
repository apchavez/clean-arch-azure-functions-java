...
- `SQL_ADMIN_PASSWORD`
- `JWT_SECRET` — HS256 secret used to sign/verify appointment endpoint JWTs (see [AuthGuard](../src/main/java/com/clinic/infrastructure/auth/AuthGuard.java)). Generate with e.g. `openssl rand -base64 32`.
...
Function-level JWT validation (`AuthGuard`) is always enforced regardless of APIM. This section is an
