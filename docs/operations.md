...
- `host.json`'s function-app-wide `retry` policy (`fixedDelay`, `maxRetryCount: 3`, `delayInterval: "00:00:10"`) — note this replaces the Java port's per-function `@FixedDelayRetry` annotation; the Python v2 model configures retry at the host level rather than per-function, so it also applies to the HTTP-triggered functions, not just the two workers.
